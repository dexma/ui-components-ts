    ...
    describe('Ancillary functions', () => {
        ...
        describe('getValueSeries', () => {
            it('should return a range serie related to the value represented on the gauge', () => {
                ...
                // When
                const resultingSerie = getValueSeries(givenProps.indicator, givenProps.max, givenProps.min, undefined, false);
                // Then
            ...
            it('should return an empty serie due to the value and threshold is outside the min max range', () => {
                ...
                // When
                const resultingSerie1 = getValueSeries(givenProps1.indicator, givenProps1.max, givenProps1.min, undefined, false);
                const resultingSerie2 = getValueSeries(givenProps2.indicator, givenProps2.max, givenProps2.min, undefined, false);
                // Then
        ...
        describe('getRangeSeries', () => {
            it('should return a sorted array of range series on ascendant order by "to" field', () => {
                ...
                // When
                const resultingRangeSeries = getRangeSeries(givenRangeSeries, Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
                // Then
            ...
            it('should return an empty array of range series as the ranges are outside the min max range', () => {
                ...
                const givenRangeSeries = mockRangeSeries;
                // When
                const resultingRangeSeries = getRangeSeries(givenRangeSeries, -100, -40);
                // Then
        ...
        describe('getCheckpointSeries', () => {
            it('should return an array of range series that represent the checkpoints on axis Y', () => {
                ...
                // When
                const resultingSerie = getCheckpointSeries(givenCheckpoint, givenProps.max, Number.MIN_SAFE_INTEGER);
                // Then
                expect(resultingSerie[0]).toStrictEqual(expectedCheckpointSerie);
            });
            it('should return an empty array range series as the checkpoints are outside the min max range', () => {
                ...
                const givenCheckpoint = mockSingleCheckpointSeries;
                // When
                const resultingSerie = getCheckpointSeries(givenCheckpoint, givenProps.max, givenProps.min);
                // Then
        ...
        describe('getYAxis', () => {
            it('should return a axisY object that defines the Axis Y on the gauge for the labels', () => {
                ...
                // When
                const resultingAxis = getYAxis(givenProps.min, givenProps.max, false);
                // Then
        ...
        describe('getChart', () => {
            it('should return a chart object that defines the chart properties on the gauge', () => {
                ...
                // When
                const resultingChart = getChart(initialChart, givenProps.indicator, givenProps.decimalSeparator, givenProps.thousandSeparator, givenProps.units, undefined, false);
                // Then
        ...
        describe('getComparisonText', () => {
            it('should return empty if comparison is disabled', () => {
                // Given
                const expectedElement = '<span style="color: #7e8084;">Compared to last seven days</span>';
                // When
        ...
        describe('indicatorLengthIsBiggerThanItsScientificNotationLength', () => {
            it('should return true when the indicator length is bigger than its equivalent scientific expression', () => {
                expect(indicatorLengthIsBiggerThanItsScientificNotationLength(12, 23445.32)).toBeTruthy();
            });
            it('should return false when the indicator length is smaller than its equivalent scientific expression', () => {
                expect(indicatorLengthIsBiggerThanItsScientificNotationLength(4, 23445.32)).toBeFalsy();
            });
        });
    });
});
