...
describe('<Card>', () => {
    ...
    it('Should call onClick event when user click on the image card', () => {
        const mockCallBack = vitest.fn();
        const { getByTestId } = render(<Card image={IMAGE_URL} onClick={mockCallBack} />);
    ...
    it('Should have a h5 when a title is set', () => {
        ...
        const { getByTestId, getByText, container } = render(<Card title={testTitle} />);
    ...
    it('Should have a h6 when a subtitle is set', () => {
        ...
        const { getByTestId, getByText, container } = render(<Card subtitle={testSubtitle} />);
        const bodyCard = getByTestId('card-body');
        expect(bodyCard).toBeTruthy();
    ...
    it('Should have a p when set a description', () => {
        ...
        const { getByTestId, getByText, container } = render(<Card description={testDescription} />);
        const bodyCard = getByTestId('card-body');
        expect(bodyCard).toBeTruthy();
